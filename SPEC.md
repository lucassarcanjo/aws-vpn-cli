...
- **Daemon supervisor.** `connect` starts `acvc-openvpn`, drives it to `CONNECTED`, records PID/socket/state so `status` and `disconnect` can control it out-of-band, and returns control to the shell. Stale state from a dead prior connection is detected and cleaned on the next `connect`.
- **Connection supervisor (drop handling).** After `CONNECTED`, `connect` registers a **per-connection root LaunchDaemon** (`awsvpn supervise`) that stays attached to the management socket and watches for the tunnel dropping (sleep, network loss). Because `acvc-openvpn` runs with `--management-hold`, a drop makes it re-enter hold rather than spin — so the supervisor sees the drop and **fails safe**: it tears the tunnel down cleanly (reverts DNS, removes routes, clears state) and posts a desktop notification so the machine is never left blackholed behind a dead tunnel. A brief `RECONNECTING` is given a short grace window to self-heal; a hold / credential prompt (which for AWS SAML means a full re-auth is required) ends it immediately. The daemon is **on-demand**: it exists only while a tunnel does — `disconnect` (and the next `connect`) boots it out — so there is no standing privileged service. It never opens a browser; a drop that needs re-auth ends in teardown + notify, and the user reconnects.
...
- **CLI.** `list`, `connect [profile]`, `disconnect`, `status`, `import <file.ovpn>`, `logs [-f]`, `version`, `install-privilege` (plus a hidden `supervise`, run only by the connection-supervisor LaunchDaemon). **Single active connection** in v1 (connecting swaps the active tunnel).
- **Presentation (`internal/ui`).** All human-facing output goes through one tiny, dependency-free layer: ANSI attributes only when the destination is a terminal that wants them (`NO_COLOR` and a non-TTY both turn everything off, so piped output and the launchd-run supervisor stay plain), plus a one-line progress reporter for `connect`. The terminal gets a narration written for a person — verified engine, engine running, waiting for sign-in, DNS applied, tunnel up — while the **connection log keeps the full machine-readable record**; `--verbose` mirrors that raw log to stderr and stops the animation so the two don't fight over a line. The reporter is an interface, so the daemon layer stays testable and silent by default. `list` and `status` also take `--json`, keeping a stable surface for agents while the human rendering is free to change.
...
- **Primary tested module — the connection reducer.** Fed scripted event sequences and asserted against emitted effects. **Fixture #1 is the management transcript captured by the spike** (challenge line, hold-release-on-restart, the redacted 10 KB `password`, `CONNECTED`), verbatim in shape with synthetic addresses and identifiers. Additional fixtures cover: hard `AUTH_FAILED` (non-CRV1) → `Failed`; SSO timeout → `Timeout` → `Failed`; SAML arriving before vs. after the second `>PASSWORD:Need` (the ordering the spike exposed).
...
- **Fake only what needs root; use the real resource for everything else.** See [ADR-0001](docs/adr/0001-fake-only-root-gated-operations.md). The operations that genuinely require root, a live endpoint, or the user's GUI session — `codesign`, exec'ing `acvc-openvpn`, `scutil`, `kill`/`ps`, opening the browser, writing the pasteboard — sit behind `system.Port` and get a recording fake in tests. Everything else costs nothing to create for real: tests use a real unix socket for the management channel, a real `t.TempDir()` for the state store, and a real loopback bind for the SAML callback. This keeps the tested code the *actual* code in `internal/mgmt`, `internal/state` and `internal/callback` rather than a stand-in for it.
- **`system.Real` itself is still not unit-tested.** It is validated end-to-end via `/verify` against a live endpoint (root + real SSO), because signature-verify/exec, `scutil` and browser open cannot be meaningfully exercised without them. The spike is the standing proof that this boundary works. What *is* unit-tested is `internal/daemon` — the ordering around that shell.
- **The connect lifecycle is transcript-tested through `daemon.Connect`.** The same captured handshake drives the whole driver, not just the reducer: the assertion is POSTed to a real callback and crosses a real socket at its real ~10KB size. Ordering guarantees are asserted as *outcomes*, never as a call sequence — "the callback port was taken, so nothing was spawned"; "the signature check failed, so nothing was spawned". Both of those also catch the corresponding reorder, because a spawn that happened before the guard is visible as a spawn that should not have occurred.
- **Not covered, deliberately:** that the signature check sits *immediately* before the exec (the TOCTOU window's width is not observable, only readable); and the keys offered during the sign-in wait (reopen the page, copy the link), because `watchKeys` reads `os.Stdin` directly and is inert under `go test`.
...
- **Background re-authentication** when the tunnel drops or the SAML session expires. The connection supervisor tears the tunnel down cleanly and notifies, but never re-runs SSO on its own (that would need to drive a browser from a root daemon into the user's session — the exact design that makes the official client spam OIDC). The user reconnects.
...
- A *standing* privileged helper / `SMAppService` daemon (always loaded, holding root when no tunnel is up) and signed/notarized prebuilt binaries as the primary install path. Note the connection supervisor above is **not** this: it is a per-connection LaunchDaemon that is booted out the moment the tunnel goes away.
